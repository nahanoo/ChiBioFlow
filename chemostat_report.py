...
def ct_oa():
    colors = {'<i>C. testosteroni</i>': '#e27e50',
              '<i>O. anthropi</i>': '#8872cd'}
    ...
    for i, d in enumerate(fig['data']):
        ...
        d['line']['color'] = colors[d['name']]
    fig.add_vline(
        x=141, col=1, annotation_text='Added Thiamine <br>to feed c=188 \u03BCM', opacity=0.5)
    fig.add_vline(x=0, annotation_text='Batch <br>culture', opacity=0.5)
    fig.add_vline(x=67, annotation_text='Start <br>dilution', opacity=0.5)
...
dfs.append(df01)
out = pd.concat(dfs)
fig = px.scatter(out,x='reactor',y='composition',color='species')
