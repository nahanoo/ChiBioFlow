...
fum = pd.read_csv('fumarate.csv')
ala = pd.read_csv('alanine.csv')
css = pd.read_csv('carbon_sources.csv')


def plot_carbon_sources():
    return px.line(css, x='time', y='OD', facet_col='carbon_source', color='strain', facet_col_wrap=6, line_group='well')


def plot_fumarate():
    px.line(fum, x='time', y='OD', color='strain', line_group='well').show()
    px.line(fum, x='time', y='growth_rates',
            color='strain', line_group='well').show()


def plot_alanine():
    px.line(ala, x='time', y='OD', color='strain', line_group='well').show()
    px.line(ala, x='time', y='growth_rates',
            color='strain', line_group='well').show()
...
def growth_paramters():
    ...
    fX = Eq(N, Y * (M - K * D / (r - D)))
    fK = solve(fX, K)[0]
    K_value = fK.subs({'N': 0.15, 'Y': 0.03, 'M': 30, 'r': 0.24, 'D': 0.16})
