...
def parse_data():
    ...
    df = fluorescence_paresr(
        join(dir, "at_oa/240531_at_oa"), down_sample=True, window_size=20, od_filter=1
    )
    to_pop = []
    ...
    fs = listdir(join(dir, "at_oa/240531_at_oa", "plate_reader", "Absorbance"))
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(dir, "at_oa/240531_at_oa", "plate_reader", "Absorbance", f),
            index_col=0,
        )
    ...
    fs = listdir(join(dir, "at_oa/240531_at_oa", "plate_reader", "GFP"))
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(dir, "at_oa/240531_at_oa", "plate_reader", "GFP", f), index_col=0
        )
        avg = np.average(data.loc["A", [1, 2, 3]])
        stdev = np.std(data.loc["A", [1, 2, 3]])
    ...
    fs = listdir(join(dir, "at_oa/240531_at_oa", "plate_reader", "mCherry"))
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(dir, "at_oa/240531_at_oa", "plate_reader", "mCherry", f),
            index_col=0,
        )
        avg = np.average(data.loc["A", [1, 2, 3]])
        stdev = np.std(data.loc["A", [1, 2, 3]])
