...
def parse_data():
    ...
    df = fluorescence_paresr(
        join(dir, "at_oa/240802_ct_oa_acetate_proline"),
        down_sample=False,
        window_size=20,
        od_filter=1,
    )
    ...
    fs = listdir(
        join(dir, "at_oa/240802_ct_oa_acetate_proline", "plate_reader", "Absorbance")
    )
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(
                dir,
                "at_oa/240802_ct_oa_acetate_proline",
                "plate_reader",
                "Absorbance",
                f,
            ),
    ...
    fs = listdir(join(dir, "at_oa/240802_ct_oa_acetate_proline", "plate_reader", "GFP"))
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(dir, "at_oa/240802_ct_oa_acetate_proline", "plate_reader", "GFP", f),
            index_col=0,
        )
        avg = np.average(data.loc["A", [1, 2, 3]])
        stdev = np.std(data.loc["A", [1, 2, 3]])
    ...
    fs = listdir(
        join(dir, "at_oa/240802_ct_oa_acetate_proline", "plate_reader", "mCherry")
    )
    for f in sorted(fs):
        t = float(f[:-5])
        data = pd.read_excel(
            join(
                dir, "at_oa/240802_ct_oa_acetate_proline", "plate_reader", "mCherry", f
            ),
            index_col=0,
        )
        avg = np.average(data.loc["A", [1, 2, 3]])
    ...
    cfus = cfu_parser("at_oa/240802_ct_oa_acetate_proline")[0]
